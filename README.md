...
read/write/socket/bind/listen/accept/close/setsockopt/exit の各 syscall は inline asm で直接発行し、ランタイムに何も要求しない。
...
- `just e2e`：実 TCP で、stdlib のみの Python WS クライアントが freestanding サーバを駆動する（echo、ping、fragment、100KB、close、不正 UTF-8 の拒否）。
